...
def train_lstm(batch_size=60,time_step=10,train_begin=0,train_end=17300):
    ...
    module_file = tf.train.latest_checkpoint('./train2')
    ...
    with tf.Session() as sess:
        ...
        for i in range(10000):
            ...
            print(i,loss_)
            if i % 1000==0:
                print("save model：", saver.save(sess, os.path.join(os.getcwd()+"\\train2", 'stock.model'), global_step=i))
...
def prediction(time_step, input_x):
    X = tf.placeholder(tf.float32, shape=[None, time_step, input_size])
    pred, _ = lstm(X)
    saver = tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        ...
        saver.restore(sess, module_file)
        test_predict = []
        prob = sess.run(pred, feed_dict={X: [input_x]})
        predict = prob.reshape(-1)
        test_predict.extend(predict)
        test_predict = np.array(test_predict) * var[input_size] + mean[input_size]
        print(test_predict)
        return test_predict[len(test_predict)-1:]

def prediction2(input_x,sess,X,pred):
    test_predict = []
    prob = sess.run(pred, feed_dict={X: [input_x]})
    predict = prob.reshape(-1)
    test_predict.extend(predict)
...
sess, X, pred =model_init()
test_x, test_y = get_test_data(10)
# print(prediction(10, test_x[0]))

print(prediction2(test_x[0], sess, X,pred))
print(prediction2(test_x[0], sess, X,pred))


@app.route('/predict/sp_500', methods=['GET', 'POST'])
def predict_sp_500():
    input_x = json.loads(request.values.get("data"))
    input_x = (input_x-mean[:input_size])/var[:input_size]
    pre = prediction2(input_x, sess, X, pred)
    return str(pre)
